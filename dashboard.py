...
def _build_overview_trend_payload_from_frames(
    ...
    return dashboard_trends.build_overview_trend_payload_from_frames(
        ld,
        ld_prev,
        hourly_candidate,
        ...
        active_overview_kpi,
        compare_active,
        paid_trend_kpi_keys=PAID_TREND_KPI_KEYS,
        kpi_catalog=KPI_CATALOG,
        platform_kpi_series_fn=_platform_kpi_series,
        ga4_kpi_series_fn=_ga4_kpi_series,
        kpi_trend_subtitle_fn=_kpi_trend_subtitle,
    )
...
def _cached_overview_trend_payload_from_report(
    ...
    return dashboard_trends.build_overview_trend_payload_from_report(
        path_str,
        modified_ns,
        size_bytes,
        start_iso,
        ...
        active_overview_kpi,
        compare_active,
        parquet_daily_dataset=PARQUET_DAILY_DATASET,
        parquet_hourly_dataset=PARQUET_HOURLY_DATASET,
        paid_trend_kpi_keys=PAID_TREND_KPI_KEYS,
        parquet_cache_signature_fn=_parquet_cache_signature,
        load_parquet_df_cached_fn=_load_parquet_df_cached,
        load_daily_df_cached_fn=_load_daily_df_cached,
        load_hourly_df_cached_fn=_load_hourly_df_cached,
        normalize_daily_table_fn=dashboard_data.normalize_daily_table,
        normalize_hourly_table_fn=dashboard_data.normalize_hourly_table,
        resolve_cached_range_fn=_resolve_cached_range,
        build_payload_from_frames_fn=_build_overview_trend_payload_from_frames,
    )
...
def render_exec(
    ...
    def _render_trend_chart() -> None:
        ...
        dashboard_trends.render_overview_trend_chart(
            st_module=st,
            payload=payload,
            active_overview_kpi=active_overview_kpi,
            platform=platform,
            ...
            kpi_catalog=KPI_CATALOG,
            kpi_trend_subtitle_fn=_kpi_trend_subtitle,
            kpi_axis_title_fn=_kpi_axis_title,
            kpi_hover_value_template_fn=_kpi_hover_value_template,
            pbi_layout_fn=pbi_layout,
    ...
    def _render_media_mix() -> None:
        dashboard_overview_sections.render_media_mix(
            st_module=st,
            platform=platform,
            df_sel=df_sel,
            sdiv_fn=sdiv,
    ...
    def _render_lead_demographics() -> None:
        dashboard_overview_sections.render_lead_demographics(
            st_module=st,
            lead_demo_df=lead_demo_df,
            platform=platform,
            start_date=s,
            end_date=e,
            prev_start_date=prev_s,
            prev_end_date=prev_e,
            age_bucket_order=AGE_BUCKET_ORDER,
            sdiv_fn=sdiv,
    ...
    def _render_lead_geo_map() -> None:
        dashboard_overview_sections.render_lead_geo_map(
            st_module=st,
            lead_geo_df=lead_geo_df,
            platform=platform,
            start_date=s,
            end_date=e,
            prev_start_date=prev_s,
            prev_end_date=prev_e,
            clean_text_value_fn=_clean_text_value,
            country_name_from_code_fn=_country_name_from_code,
            theme_color_scale_fn=_theme_color_scale,
            sdiv_fn=sdiv,
            pct_delta_fn=pct_delta,
            fmt_delta_compact_fn=fmt_delta_compact,
            fmt_pct_fn=fmt_pct,
...
def render_top_pieces_range(
    ...
    *,
    ...
    return dashboard_overview_sections.render_top_pieces_range(
        st_module=st,
        camp_df=camp_df,
        piece_df=piece_df,
        platform=platform,
        ...
        report_cache_sig=report_cache_sig,
        campaign_filter_options=CAMPAIGN_FILTER_OPTIONS,
        apply_campaign_filters_fn=_apply_campaign_filters,
        campaign_filters_cache_key_fn=_campaign_filters_cache_key,
        cached_top_pieces_roll_from_report_fn=_cached_top_pieces_roll_from_report,
        piece_platform_link_fn=piece_platform_link,
        campaign_platform_link_fn=campaign_platform_link,
        sdiv_fn=sdiv,
    )
